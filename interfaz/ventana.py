...
def abrir_f1(raiz):
    ...
    tk.Button(v, text="Generar", bg="#E0E0E0", command=ejecutar).grid(
...
def abrir_f5(raiz):
    ...
    tk.Label(v, text="Frase base (mín 5 palabras):").grid(
        row=0, column=0, padx=10, pady=10, sticky="w")
    entry_frase = tk.Entry(v)
    entry_frase.grid(row=0, column=1, padx=10, pady=10, sticky="ew")

    tk.Label(v, text="Símbolo final:").grid(
...
def abrir_f8_f9(raiz):
    ...
    tk.Button(v, text="Evaluar Contraseña", bg="#E0E0E0", command=ejecutar).grid(
        row=1, column=0, columnspan=2, pady=10)
...
def abrir_f10(raiz):
    ...
    def ejecutar():
        ...
        try:
            ...
            resultado_ataque = ataques.simulacion_fuerza_bruta_recursiva(
                obj, chars, lon_max)
            messagebox.showinfo("Simulación Completa",
                                resultado_ataque, parent=v)
        except ValueError:
            messagebox.showerror(
                "Error", "Ingrese una longitud máxima entera válida.", parent=v)

    tk.Button(v, text="Iniciar Simulación", bg="#E0E0E0", command=ejecutar).grid(
        row=3, column=0, columnspan=2, pady=10)


def abrir_f13(raiz):
    v = tk.Toplevel(raiz)
    v.title("Función 13: Leetspeak Compuesta")
    v.geometry("450x250")
    v.columnconfigure(1, weight=1)

    tk.Label(v, text="Frase (mín 3 palabras):").grid(
        row=0, column=0, padx=10, pady=10, sticky="w")
    entry_frase = tk.Entry(v)
    entry_frase.grid(row=0, column=1, padx=10, pady=10, sticky="ew")
    ...
    tk.Label(v, text="Número final:").grid(
        row=1, column=0, padx=10, pady=10, sticky="w")
    entry_numero = tk.Entry(v)
    entry_numero.grid(row=1, column=1, padx=10, pady=10, sticky="ew")
    ...
    def ejecutar():
        try:
            frase = entry_frase.get()
            numero_final = int(entry_numero.get())
            res = generador.generar_contraseña_leetspeak(frase, numero_final)
            entry_res.delete(0, tk.END)
            entry_res.insert(0, res)
        except ValueError as error:
            messagebox.showerror("Error", str(error), parent=v)
...
def iniciar_app():
    ...
    tk.Label(ventana, text="GESTOR DE CONTRASEÑAS", font=(
        "Arial", 13, "bold")).grid(row=0, column=0, columnspan=2, pady=15)

    tk.Button(ventana,
              text="F1: Numérica Simple",
              width=25,
              command=lambda: abrir_f1(ventana)).grid(row=1, column=0, padx=15, pady=6, sticky="ew")
    tk.Button(ventana,
              text="F2: Alfabética Simple",
              width=25,
              command=lambda: abrir_f2(ventana)).grid(row=1, column=1, padx=15, pady=6, sticky="ew")
    tk.Button(ventana,
              text="F3: Alfanumérica",
              width=25,
              command=lambda: abrir_f3(ventana)).grid(row=2, column=0, padx=15, pady=6, sticky="ew")
    tk.Button(ventana,
              text="F4: Contraseña Robusta",
              width=25,
              command=lambda: abrir_f4(ventana)).grid(row=2, column=1, padx=15, pady=6, sticky="ew")
    tk.Button(ventana,
              text="F5: Basada en Frase",
              width=25,
              command=lambda: abrir_f5(ventana)).grid(row=3, column=0, padx=15, pady=6, sticky="ew")
    tk.Button(ventana,
              text="F6: Frase Aleatoria",
              width=25,
              command=lambda: abrir_f6(ventana)).grid(row=3, column=1, padx=15, pady=6, sticky="ew")
    tk.Button(ventana,
              text="F7: Passphrase",
              width=25,
              command=lambda: abrir_f7(ventana)).grid(row=4, column=0, padx=15, pady=6, sticky="ew")
    tk.Button(ventana,
              text="F13: Leetspeak Compuesta",
              width=25,
              command=lambda: abrir_f13(ventana)).grid(row=4, column=1, padx=15, pady=6, sticky="ew")

    tk.Label(ventana, text="Módulos de Análisis y Ataque", font=(
        "Arial", 10, "italic")).grid(row=5, column=0, columnspan=2, pady=15)

    tk.Button(ventana, text="F8/F9: Evaluar Fortaleza y Patrones",
              command=lambda: abrir_f8_f9(ventana)).grid(row=6, column=0, columnspan=2, padx=15, pady=6, sticky="ew")
    tk.Button(ventana, text="F10: Simulación Fuerza Bruta", command=lambda: abrir_f10(ventana)).grid(
        row=7, column=0, columnspan=2, padx=15, pady=6, sticky="ew")
